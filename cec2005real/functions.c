...
long double calc_ackley (long double *x, int nreal) {
	int i;
...
long double calc_rastrigin (long double *x, int nreal) {
	int i;
	long double res;
	res = 0.0;
	for (i = 0; i < nreal; i++) {
...
long double calc_weierstrass (long double *x, int nreal) {
	int i, j, k_max;
	long double res, sum, a, b;
	a = 0.5, b = 3.0, res = 0.0;
	k_max = 20;
	for (i = 0; i < nreal; i++) {
		sum = 0.0;
...
long double calc_griewank (long double *x, int nreal) {
	int i;
	long double s, p, res;
	s = 0.0, p = 1.0;
	for (i=0; i < nreal; i++) {
...
long double calc_sphere (long double *x, int nreal) {
	int i;
	long double res;
	res = 0.0;
	for (i = 0; i < nreal; i++) {
...
long double calc_schwefel (long double *x, int nreal) {
	int i, j;
...
long double calc_rosenbrock (long double *x, int nreal) {
	int i;
	long double res;
	res = 0.0;
	for (i = 0; i < nreal - 1; i++) {
...
long double nc_schaffer (long double x, long double y) {
	...
	for (i=0; i<2; i++) {
		if (fabs(t1[i]) >= 0.5) {
			res = 2.0*t1[i];
			// Dani: Cambiado para eliminar warning
			a = (int) res;
			b = fabs(res - a);
			if (b < 0.5) {
				t2[i] = a / 2.0;
...
long double nc_rastrigin (long double *x, int nreal, long double *temp_x4) {
	int i;
	int a;
	long double b;
	long double res;
	for (i = 0; i < nreal; i++) {
		if (fabs(x[i]) >= 0.5) {
			res = 2.0 * x[i];
			// Dani: Cambiado para eliminar el warning
			a = (int) res;
			b = fabs(res - a);
			if (b < 0.5) {
				temp_x4[i] = a / 2.0;
